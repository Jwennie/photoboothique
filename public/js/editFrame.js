...
/* ═══════════════════════════════════════════════════════════
   ...
   Layer order (bottom → top):
     z-index 1 → frameSlots  (photos live here)
     z-index 2 → frame PNG   (transparent holes reveal photos below)
     z-index 5 → stickersLayer
     z-index 6 → frameFooter
═══════════════════════════════════════════════════════════ */
function buildFrame() {
    const cfg = st.cfg;

    // Remove old PNG overlay if rebuilding
    ...
    // ── PNG frame overlay — z-index 2, sits ABOVE photos ──
    // Because the PNG has transparent holes, photos show through.
    // pointer-events: none so clicks still reach the slots below.
    ...
    frameOuter.appendChild(framePng);
...
function goBack() {
    ...
    window.location.href = mode === 'upload' ? '/upload-photo' : '/choose-session'
}
